...
export default class CFScreen extends React.Component {
  static navigationOptions = {
    title: 'DE LANNA RIVERFRONT',
  }
  state={
    item:[]
  }
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('dlr').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Dlrc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Dlrc').push(data).then(()=>{
