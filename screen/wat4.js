...
export default class Wat4Screen extends React.Component {
  static navigationOptions = {
    title: 'Wat Ming Mueang ',
  }
  state={
    item:[]
  }
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('wmm').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Wmmc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Wmmc').push(data).then(()=>{
