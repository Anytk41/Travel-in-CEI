...
export default class PM1Screen extends React.Component {
...
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('ka').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Kac').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Kac').push(data).then(()=>{
