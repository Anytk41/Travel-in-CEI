...
export default class CF1Screen extends React.Component {
  static navigationOptions = {
    title: 'THE WANDERER CAFE & RESTAURANT',
  }
  state={
    item:[]
  }
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('wcr').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Wcrc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Wcrc').push(data).then(()=>{
