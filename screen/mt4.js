...
export default class MT4Screen extends React.Component {
  static navigationOptions = {
    title: 'Doi Hua Mae Kham',
  }
  state={
    item:[]
  }
constructor(props){
  super(props)
  this.state = {name : '',address : '',info : '',time : '',travel : ''}
}

  componentDidMount(){
    firebase.database().ref('dhmk').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Dhmkc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Dhmkc').push(data).then(()=>{
