...
export default class Wat1Screen extends React.Component {
...
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('wpk').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Wpkc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Wpkc').push(data).then(()=>{
