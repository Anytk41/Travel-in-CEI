...
export default class MT3Screen extends React.Component {
...
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('dpt').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Dptc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Dptc').push(data).then(()=>{
