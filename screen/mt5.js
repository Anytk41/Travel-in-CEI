...
export default class MT5Screen extends React.Component {
...
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('dw').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Dwc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Dwc').push(data).then(()=>{
  ...
  if(this.state.name.trim() !== ''){
    ...
    return (
      ...
      <View style={{flex:1, alignItems:'center', justifyContent:'center'}}>
          <Image source={{uri:item.image}} style={{width:300,height:180}}/></View>
