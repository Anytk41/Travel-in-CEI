...
export default class PM4Screen extends React.Component {
...
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('ok').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Okc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Okc').push(data).then(()=>{
