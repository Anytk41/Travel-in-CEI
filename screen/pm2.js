...
export default class PM2Screen extends React.Component {
  static navigationOptions = {
    title: 'MAE FAH LUANG ART AND CULTURE PARK',
  }
  state={
    item:[]
  }
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('mfu').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Mfuc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Mfuc').push(data).then(()=>{
