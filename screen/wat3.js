...
export default class Wat3Screen extends React.Component {
  static navigationOptions = {
    title: 'Wat Phra Dhat Pha-Ngao',
  }
  state={
    item:[]
  }
constructor(props){
  ...
  componentDidMount(){
    firebase.database().ref('wpdpn').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
      this.setState({data:item})
    })
    firebase.database().ref('Wpdpnc').on('value', snap =>{
      var data = snap.val()
      var item = Object.values(data)
 ...
 firebase.database().ref('Wpdpnc').push(data).then(()=>{
